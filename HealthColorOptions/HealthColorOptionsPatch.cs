...
namespace Grimoire.HealthColorOptions
{
    ...
    internal class HealthColorOptionsPatch
    {
        ...
        [HarmonyPrefix]
        private static void CreateHealthOptionChangeButton_Prefix(CombatVisualizationController __instance)
        {
        ...
        [HarmonyPrefix]
        private static void UpdateHealthOptionChangeButton_Prefix(CombatHealthBarLayout __instance)
        {
            ...
            var buttonActive = u != null && u.GetHealthColorOptions().Count > 1;
        ...
        [HarmonyPostfix]
        private static void UpdateHealthOptionChangeButtonInteractivity_Postfix(AttackCostLayout __instance)
        {
            if (CombatManager.Instance == null || CombatManager.Instance._combatUI == null)
                return;
        ...
        [HarmonyPostfix]
        private static void UpdateCurrentHealthOption_Postfix(IUnit __instance, ManaColorSO value)
        {
            var holder = __instance.HealthOptionHolder();

            // If a unit's health color gets updated, also update its current health option to that color
            holder.healthColors[holder.currentHealthColor % holder.healthColors.Count] = value;
        }
        ...
        private class ChangeHealthColorButtonHolder : MonoBehaviour
        {
            ...
            public void ButtonClicked()
            {
                ...
                var hold = u.HealthOptionHolder();
                var origIdx = hold.currentHealthColor;

                // Switch to the next health color
                hold.currentHealthColor = (hold.currentHealthColor + 1) % hold.healthColors.Count;

                if (u.ChangeHealthColor(hold.healthColors[hold.currentHealthColor]))
                    return;

                // If the health color change wasn't successful, revert to the original health color index
                hold.currentHealthColor = origIdx;
            }
        }
    }
}
